...
>
> **Fase 8 chiusa sullo staging il 23/09/2026**: il plugin funziona nella pagina Elementor vera, provato su iPhone/Safari e Chrome/Android. **In produzione è installato e attivo dal 23/09/2026, alla 1.6.1 (1.6.2 dallo stesso giorno: autore «Issimissimo»), per decisione di Daniele** — con l'invio acceso, prima dei passi 4-6 (rate limit, retention, testi per i genitori), avvisato del rischio. **Il rate limit (passo 4) è in produzione dal 25/09/2026, alla 1.8.0**; restano scoperti retention e testi per i genitori. Lo stato approvato dal cliente prima di quel passo è il tag `approvato-cliente-25092026`. `admin_email` in produzione è `d.suppo@issimissimo.com`. Si aggiorna con `python .lavoro/installa-staging.py <versione> --produzione`. Le tre decisioni che la aprivano sono sciolte: **pagina normale con header** (non Canvas), **iframe** (non inline), **staging SiteGround**. Le fasi 5, 6, 7, 9, 10 restano fuori perimetro: SALVA continua a scaricare e non a inviare.
>
