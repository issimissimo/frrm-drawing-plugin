...
>
> **Fase 8 chiusa sullo staging il 23/09/2026**: il plugin funziona nella pagina Elementor vera, provato su iPhone/Safari e Chrome/Android. **In produzione è installato e attivo dal 23/09/2026, alla 1.6.1 (1.6.2 dallo stesso giorno: autore «Issimissimo»), per decisione di Daniele** — con l'invio acceso, prima dei passi 4-6 (rate limit, retention, testi per i genitori), avvisato del rischio. **Il rate limit (passo 4) è in produzione dal 25/09/2026, alla 1.8.0, e la retention (passo 5) dal 26/09/2026, alla 1.9.0**: eliminare un disegno cancella anche la sua immagine, che prima restava orfana e **leggibile da anonimo** in `/wp/v2/media`. Resta scoperto il 6 (testi per i genitori), sospeso. **Dalla 1.10.0, solo staging**: la mail di notifica ha i tasti **Approva / Rifiuta** (link firmato, 7 giorni, agisce solo la conferma in POST) e va all'impostazione «Notifiche dei disegni», non ad `admin_email`: sarà il cliente, che in WordPress non entra. Regole nel README del plugin. Lo stato approvato dal cliente prima di quel passo è il tag `approvato-cliente-25092026`. `admin_email` in produzione è `d.suppo@issimissimo.com`. Si aggiorna con `python .lavoro/installa-staging.py <versione> --produzione`. Le tre decisioni che la aprivano sono sciolte: **pagina normale con header** (non Canvas), **iframe** (non inline), **staging SiteGround**. Le fasi 5, 6, 7, 9, 10 restano fuori perimetro: SALVA continua a scaricare e non a inviare.
>
