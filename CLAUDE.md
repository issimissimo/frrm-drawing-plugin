...
>
> **Fase 8 chiusa sullo staging il 23/09/2026**: il plugin funziona nella pagina Elementor vera, provato su iPhone/Safari e Chrome/Android. **In produzione è installato e attivo dal 23/09/2026, alla 1.6.1 (1.6.2 dallo stesso giorno: autore «Issimissimo»), per decisione di Daniele** — con l'invio acceso, prima dei passi 4-6 (rate limit, retention, testi per i genitori), avvisato del rischio. **Il rate limit (passo 4) è in produzione dal 25/09/2026, alla 1.8.0, e la retention (passo 5) dal 26/09/2026, alla 1.9.0**: eliminare un disegno cancella anche la sua immagine, che prima restava orfana e **leggibile da anonimo** in `/wp/v2/media`. Resta scoperto il 6 (testi per i genitori), sospeso. **In produzione dal 26/09/2026, alla 1.11.5, la moderazione dalla mail**: la mail di notifica ha un link firmato (7 giorni) a una pagina con **Approva / Rifiuta**, e agisce solo il tasto in POST. Va all'impostazione «Notifiche dei disegni», non ad `admin_email`: sarà il cliente, che in WordPress non entra. In produzione quell'impostazione è ancora vuota, quindi le mail vanno ad `admin_email`. Regole nel README del plugin. Lo stato approvato dal cliente prima di quel passo è il tag `approvato-cliente-25092026`. `admin_email` in produzione è `d.suppo@issimissimo.com`. Si aggiorna con `python .lavoro/installa-staging.py <versione> --produzione`. Le tre decisioni che la aprivano sono sciolte: **pagina normale con header** (non Canvas), **iframe** (non inline), **staging SiteGround**. Le fasi 5, 6, 7, 9, 10 restano fuori perimetro: SALVA continua a scaricare e non a inviare.
>
...
Registra lo shortcode `[lavagna]`, che stampa un `<iframe>` verso l'app. Nella pagina ospite non finisce niente della lavagna: né JS, né CSS, né font. **Dalla 1.3.0** (23/09/2026, solo staging) riceve anche i disegni: `POST /wp-json/frmm-lavagna/v1/invio`, codice in `includes/`, regole da non disfare nel README del plugin. **Dalla 1.5.x** l'app li manda: SALVA chiede **prima** «SALVA E INVIA» / «SOLO SALVA» — dopo, chi è finito in WhatsApp non risponde — e l'invio parte nello stesso tocco della condivisione, con ripresa automatica e `invio_id` contro i doppioni (`prototipo/src/invio.js`). L'indirizzo arriva dallo shortcode in `?invio=`. **Dalla 1.8.0** c'è il rate limit: **3 disegni per dispositivo e 20 per IP in 24 ore**, oltre `429` e l'app lascia perdere in silenzio (`includes/limiti.php`). Conta **solo su `REMOTE_ADDR`, mai su `X-Forwarded-For`**, misurato dietro la CDN di SiteGround: sullo staging quell'intestazione arriva come la scrive il client. Regole nel README del plugin. Si installa sullo staging con `python .lavoro/installa-staging.py`, che legge le credenziali da sé.
...
**Online:** <https://issimissimo.com/temp/frmm-drawing-plugin-19/> — con `?tutorial` in coda il tutorial parte comunque. Fuori da WordPress la domanda d'invio non compare (manca `?invio=`): la -19 è la -18 con la correzione dei gessetti che su Firefox desktop finivano sotto il cancellino (26/09/2026); la -18 è la -17 con il tratto a opacità 0.35 (23/09/2026, cliente); la -17 è la -14 più la correzione di `adattaLavagna()` (la -16 è la stessa cosa col codice della domanda *dopo*, inattivo fuori da WordPress). La **-15** è la pagina ospite di prova della Fase 8. La **-13** è la -14 **senza il logo** sull'immagine scaricata.
