...
- **L'arancione istituzionale `#FF6000` vive solo dentro il tutorial** (cliente, 18/09/2026): tratteggio animato, etichetta «PASSO n DI 7», testo e bordo di AVANTI / HO CAPITO. **Nella mensola non entra**, ed è la decisione del 17/09 che regge ancora: lì nove gessetti portano informazione col colore, e un tasto arancione pieno diventerebbe l'elemento più colorato dello schermo senza dire nulla. Sotto il velo del tutorial i gessetti sono spenti, quindi l'obiezione non si applica e l'arancione resta l'unica cosa accesa — che è il punto.

  Conseguenza da non disfare: le regole del tutorial portano **l'id** (`#btn-avanti`, `#btn-ripeti`) e non `.primario` / `.secondario`, perché **SALVA condivide quelle classi**. Scriverle sulle classi colora la mensola.
