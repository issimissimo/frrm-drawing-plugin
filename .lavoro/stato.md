...
- **Fuori dal piano, chiesto da Daniele**: moderazione dalla mail (1.10.0 → 1.11.2, **solo staging**). Fatto e provato. **In corso**: Daniele ritocca l'aspetto della pagina sul design <https://claude.ai/artifact/QZApsX3hLAqZtKDwwbsQoN> (4 tavole; font Nunito al posto di SebinoSoft); le modifiche vanno portate a mano in `moderazione-mail.php`. **Per la produzione servono**: l'indirizzo del cliente (`b.bruschi@monaco.mc`, personale) nell'impostazione «Notifiche dei disegni», e una prova del link da telefono in produzione (CDN + firewall).
